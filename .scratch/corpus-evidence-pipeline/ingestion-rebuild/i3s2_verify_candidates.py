...
def run_self_consistency(
    ...
    for question in gold:
        ...
        if status in {"machine_ready", "pending_human"}:
            ok = item.doc_recall == 1 and item.question_pass is True and item.evidence_pass is True
                ...
                "targets": [
                    {
                        ...
                        "constraints": {},
                        "basis": {"gold_id": "probe-slot", "item_index": 0},
                    },
                ],
                "pending_human": [
                    {
                        "item_id": "I32-probe-q1-01",
                        "kind": "qualification",
                        "facet_id": "q2",
                        "clause_id": "c1",
                        "question": "确认门槛口径",
    ...
    slots = [
        {
            ...
            "expected_items": [
                ...
                {"kind": "condition", "quote": "门槛口径为年度累计", "text": "门槛口径为年度累计"},
            ],
        },
    ]
        ...
        "facet_decisions": [
            {
                ...
                "query_id": "probe-q1",
                "decision": "批准",
                "chosen": [{"slot": "probe-slot", "item_index": 1}],
...
def run_probes(
    ...
    obs = _observation(
        ...
        [
            (
                str(targets[1]["source_id"]),
                str(targets[1]["quote"]),
                tuple(str(token) for token in targets[1]["locator"]),
                True,
            ),
            (
                str(targets[2]["source_id"]),
                str(targets[2]["quote"]),
                tuple(str(token) for token in targets[2]["locator"]),
                True,
            ),
        ],
    )
    ...
    obs = _observation(
        ...
        [
            (
                str(note1["source_id"]),
                str(note1["quote"]),
                tuple(str(token) for token in note1["locator"]),
                True,
            )
        ],
    )
    ...
    obs = _observation(
        ...
        [
            (
                ...
                str(target["quote"]),
                tuple(str(token) for token in target["locator"]),
                True,
            )
            for target in notes
        ],
    ...
    _record(
        ...
        "company-005 的第2页型号清单属补充证据（非必需），缺失不得判失败（r26 期望值随补证采纳更新：新增 p3 槽位两条补充目标）",
    ...
    obs = _observation(
        ...
        [
            (
                str(main["source_id"]),
                str(main["quote"]),
                tuple(str(token) for token in main["locator"]),
                True,
            )
        ],
    )
    ...
    target = next(target for target in candidate["targets"] if "13.4" in str(target["quote"]))
    _record(
    ...
    if "I32-macro-004-03" in {
        str(entry["item_id"]) for entry in candidates["macro-004"]["pending_human"]
    }:
        entry = next(
            item
            for item in candidates["macro-004"]["pending_human"]
    ...
    _record(
        ...
        gate_ready["ready"] is True and len(projection["questions"][0]["approved_required"]) == 2,
        {
            "ready": gate_ready["ready"],
            "blockers": gate_ready["blockers"],
...
def main() -> int:
    ...
    for item in self_checks:
        if not item["passed"]:
            print(
                f"  SELF FAILED {item['query_id']} ({item['machine_status']}): {item['failures']}"
            )
    for item in probes:
        if not item["passed"]:
