...
def prepare():
    ...
    write_once("calibration-plan.json", plan)
...
def main():
    if "--prepare" in sys.argv:
        prepare()
        return
    plan = json.loads((HERE / "calibration-plan.json").read_text())
    ...
    from plugins.corpus.preparation.read_pg import fetch_verbatim, document_handle, chunk_locator
    import psycopg
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        def fetch(hit):
            ...
            if key not in cache:
                cache[key] = fetch_verbatim(dsn, document_handle(hit.build_id), chunk_locator(hit.chunk_id))
