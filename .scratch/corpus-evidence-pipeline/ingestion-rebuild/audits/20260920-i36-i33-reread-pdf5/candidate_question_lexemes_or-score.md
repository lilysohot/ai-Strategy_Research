...
逐类指标（判定按类逐项，不用总体平均掩盖弱类）
  ...
  [macro] 有答案题 8 | DocRecall@5 宏平均 100.0%（满分 8/8） | QuestionPass 8/8=100.0% | EvidencePass 3/8=37.5%
  [合计] 类间宏平均（各类等权） DocRecall 100.0% | QuestionPass 100.0% | EvidencePass 29.2%
         题数汇总（非类间平均） QuestionPass 24/24、EvidencePass 7/24
  负例误报 6 条、伪造引用 6 条（另计，不混入召回分母）

阻断项（存在即不得宣告通过）
  below_threshold:company:evidence_pass=12.5%
  below_threshold:industry:evidence_pass=37.5%
  below_threshold:macro:evidence_pass=37.5%
