...
**2026-09-23 当前 M6 状态（i0c-r5e）：产品真实链路重新过门。**
冻结 30 题直接调用注册 `corpus_search/corpus_fetch`，不按标签分支、不修改 gold、评分器或
95% 阈值：默认原题干、`limit=10`、abstain 默认开启时，QP **24/24**、EP **24/24**、
6 个负例 false positive **0**、工具失败 0。检索侧采用实质词元 OR 候选、来源标题召回、
每来源 40 候选和有界同页证据带；取证侧按原文序抓取 context locators，并以 authority
unit span 校验结构化 cell；拒答侧只收紧明确询问“材料/报告是否提供某事实”的请求，
普通研究问题不进入严格门。隔离库重建恢复 8/8 active；全仓 2961 passed，剩余 2 项为
既有跨模块失败。r5d 的失败结论和产物保持不变，r5e 作为后续追加修订绑定当前实现与证据。
见[整改报告](../../.scratch/m6-retrieval-fix-20260923/report.md)。

**2026-09-23 当前 M6 状态（i0c-r5d）：实现整改已验证，正式产品门未通过。**
本轮修复上下文坏哈希未拒绝、正式 fetch 缺失补充证据、验收观测脱离真实工具路径三项问题；
保留 r5c 历史签认和所有旧实验产物。新采集器以真实 `corpus_search/corpus_fetch` 完整执行
30 题：默认原题干 limit=10 的 QP/EP 为 0/24；所有题同样显式 OR、limit=20 的诊断为
QP 22/24、EP 13/24、负例误报 6/6。未修改金标、评分器、95% 阈值、索引或产品查询策略；
abstain=on 正例误拒仍属既有限制。新版本只冻结修复与真实失败证据，不能用旧实验 24/24
代替当前产品验收，**I4 不因此获得放行**。后续应独立定义并验证统一检索/拒答策略及证据预算。
见[修复与回归报告](../../.scratch/m6-repair-20260923/report.md)。
...
| I4 | 经复核批准的 reset/migrate 条件切换；defer 则不执行                          | I3 + 停写截止点后最终备份恢复 + 精确切换清单/批准  | **已执行完毕并放行（2026-09-24，i0c-r5n U 具名签认）**：migrate\_then\_reset\_limited 按批准双 manifest 执行——migrate 九表（一次受控回滚后重跑绿）、reset 阶段 1 七表 + 阶段 2 六表 TRUNCATE 五门全过；I4-4 重建 8/8 与 I3-7 基线同、I4-5 真工具往返 + 旧写入口停用；旧九表归零，原文/人工/审计与保留序列/见证不变；详见 [I4 窗口报告](../../.scratch/corpus-evidence-pipeline/ingestion-rebuild/audits/20260923-i4-window/report.md) 与 [M7 签认记录](../../.scratch/m7-rereview-20260924/signoff-record-m7.md)                                                                                                                                                                                                                                                                                                                                                                                                      |
| I5 | 增量、版本升级重建与维护文档，退役旧入口                                          | I4                             | **I5-1 已执行（2026-09-24，四场景全绿，i0c-r5o）**：验证范围经 U 批准为**全四场景 × 全部 8 活动源**（同源同版本重跑、源变化、新规则 build、复用解析重建索引；源变化/新规则/复用解析在隔离环境执行，不触碰生产活动来源），与 I2/I3 同场景结果对照全绿（[总报告](../../.scratch/corpus-evidence-pipeline/ingestion-rebuild/audits/20260924-i51-scenarios/i51-summary-report.json)；发现 F1：检查点有效重跑下动态 rev 读取器 build_id 翻转，登记待 U 裁定）；I5-2 维护说明与 I5-3 退休核验（含 stats/list\_documents 迁移残留处置）待 F1 裁定后                                                                                                                                                                                                                                                                                                                                                                    |
