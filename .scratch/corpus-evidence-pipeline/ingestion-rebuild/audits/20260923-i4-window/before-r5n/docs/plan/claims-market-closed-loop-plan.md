...
| I5 | 增量、版本升级重建与维护文档，退役旧入口                                          | I4                             | **已启动（2026-09-24）**：I5-1 验证范围经 U 批准为**全四场景 × 全部 8 活动源**（同源同版本重跑、源变化、新规则 build、复用解析重建索引；源变化/新规则/复用解析在隔离环境执行，不触碰生产活动来源），与 I2/I3 同场景结果对照；门：幂等、版本、缓存及失败恢复正确，无双写；I5-2 维护说明与 I5-3 退休核验（含 stats/list\_documents 迁移残留处置）待 I5-1 后                                                                                                                                                                                                                                                                                                                                                                    |
