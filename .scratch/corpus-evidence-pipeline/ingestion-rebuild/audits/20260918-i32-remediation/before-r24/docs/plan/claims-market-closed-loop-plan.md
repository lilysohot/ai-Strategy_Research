...
| I0—I5 | [语料基础链路重构](corpus-ingestion-rebuild-architecture.md)                                                                                                                                                                              | PR-DATA-01/02/09/12、PR-OUT-04/06/07 | 三类范围已明确；候选设计待 I0 证据复核        | **I0-A 交付齐（M1 达成）、I0-B 完成（M2 证据齐备）、I0-C 完成（M3 达成）、I1/M4 放行、I2 全部任务完成且 M5 已独立复核放行并经 U 签认（2026-09-18）；I3-0 已交付并按独立复核 F1—F5 整改（复核探针 10/10、自身 46 passed，待复核确认闭环）；I3-2 补料候选已按两轮独立复核（F1—F5、A1—A5）整改（规则 `evidence-mapping-5` + 裁决应用器：30 题 → machine_ready 2／pending_human 20／blocked 2／负例 6，目标 54 条；自洽 0 failed、独立反例 16/16、审批门 no_decisions 待 40+24+6+1 项人工裁决）；M6—M8 未放行**；强制零模型，生产库未写入/未清库，不依赖 R2 语义通过 |
...
1. **要件裁决 40 项**（裁决单 §2）：定性锚点（`partial` 须改选/补标/写 residual）、数值等价、
   多来源覆盖、答案侧口径；
...
**状态**：I3-2 补料候选（A 侧，第三轮）+ 二轮复核 A1—A5 整改完成；**未冻结金标、未宣告 I3-2 完成**
