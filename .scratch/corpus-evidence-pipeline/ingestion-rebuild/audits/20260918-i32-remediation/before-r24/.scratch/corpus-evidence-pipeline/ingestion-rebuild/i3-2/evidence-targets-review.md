...
- 生成时间：2026-09-18T16:39:28+08:00；规则版本：`evidence-mapping-5`
- 输入：`query-gold-frozen.jsonl` sha256=6f6c5a25d55b…、`source-gold-frozen.jsonl` sha256=c360a132ad63…
- 机器状态：machine_ready **2** ／ pending_human **20** ／ blocked **2** ／ 负例 **6**
- 目标：必需 **35** ／ 补充（非必需）**4** ／ 待批准锚点 **15**（合计 54 条）
...
- 机器判定：机器已定位全部数值要件（必需 1 条、补充 3 条），但仍有 2 项需人工裁决（定性要件锚点/数值等价/多来源覆盖/答案侧口径）；批准前不得计入必需证据，且机器状态不等于人工已核验
...
- evidence_required：`true`；候选文件内状态：`pending`（信息字段，不作批准凭据）
- 机器判定：机器已定位全部数值要件（必需 6 条、补充 0 条），但仍有 2 项需人工裁决（定性要件锚点/数值等价/多来源覆盖/答案侧口径）；批准前不得计入必需证据，且机器状态不等于人工已核验
- 页码提示（不参与匹配）：page:20
...
- evidence_required：`true`；候选文件内状态：`pending`（信息字段，不作批准凭据）
- 机器判定：机器已定位全部数值要件（必需 1 条、补充 0 条），但仍有 2 项需人工裁决（定性要件锚点/数值等价/多来源覆盖/答案侧口径）；批准前不得计入必需证据，且机器状态不等于人工已核验
- 候选槽位：company-003-claim-001、company-018-claim-001、company-024-claim-001、company-028-claim-001、company-032-claim-001、company-036-claim-001、company-040-claim-001、company-044-claim-001、company-048-claim-001、company-052-claim-001、company-056-claim-001、company-060-claim-001
- 要求覆盖账（按段）：
...
- evidence_required：`true`；候选文件内状态：`pending`（信息字段，不作批准凭据）
- 机器判定：机器已定位全部数值要件（必需 1 条、补充 0 条），但仍有 1 项需人工裁决（定性要件锚点/数值等价/多来源覆盖/答案侧口径）；批准前不得计入必需证据，且机器状态不等于人工已核验
- 候选槽位：company-001-claim-001、company-008-claim-001、company-013-claim-001
- 要求覆盖账（按段）：
    - `c1` '茅台集团持有华创云信4.06%股份'
        - 数值要件 `f1` '4.06%' → required
    - `c2` '华创云信为华创证券控股股东，不能误记成华创证券持有茅台集团'
        - 定性要件 `q2` → usage_constraint（锚点 1 条）
- 必需证据（计入 EvidencePass）：
...
- 待批准锚点（批准前不得计入必需）：
    - `s1` 2026-08-16_6f14cc14 page:1 ← company-008-claim-001#4（rating；约束 {'unit': '元', 'period': '报告日2026-08-16；目标价期限一年'}）
        - quote：'我们维持26-28 年EPS 预测值\n67.74/70.77/73.84 元，维持一年目标价2030 元和“强推”评级。'
        - **部分覆盖，未覆盖词元：光力、力科、大市、州茅、次覆、科技、茅台、覆盖、贵州、首次**
...
- 要求覆盖账（按段）：
    ...
    - `c3` '不是所有电子特气共同涨幅，也不是单日涨幅'
        - 定性要件 `q4` → usage_constraint（锚点 0 条）
- 必需证据（计入 EvidencePass）：
...
- 要求覆盖账（按段）：
    - `c1` '报告援引供应链消息：计划用于NVSwitch板卡及Rubin Ultra正交背板主力选材'
        - 定性要件 `q1` → suggested（锚点 1 条）
    - `c2` 'Rubin Ultra预计2027年推出'
        - 定性要件 `q2` → suggested（锚点 1 条）
...
- evidence_required：`true`；候选文件内状态：`pending`（信息字段，不作批准凭据）
- 机器判定：机器已定位全部数值要件（必需 1 条、补充 0 条），但仍有 3 项需人工裁决（定性要件锚点/数值等价/多来源覆盖/答案侧口径）；批准前不得计入必需证据，且机器状态不等于人工已核验
...
- 待批准锚点（批准前不得计入必需）：
    - `s1` 2026-09-06_cc03f55b page:1 ← macro-039-claim-001#0（condition；约束 {}）
        ...
        - **部分覆盖，未覆盖词元：依赖、偏低、入依、合税、堵点、拖累、收入、税负、综合、行拖、负偏、赖旧**
- 答案侧约束（不产生证据目标，核验落点 I3-5）：
    - `q3` '须标注预测'
- 要件裁决队列（2 项）：
