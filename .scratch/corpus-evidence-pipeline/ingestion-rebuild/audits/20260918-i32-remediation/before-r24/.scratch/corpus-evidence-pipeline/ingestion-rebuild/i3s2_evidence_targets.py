...
def _suggestion(
    ...
    missing = uncovered_terms(segment, item, frequency, total_items)
    return {
        "slot": str(slot.get("gold_id")),
        "source_id": str(slot.get("source_id")),
        "item_index": index,
    ...
    if notes:
        ...
        for slot in pool:
            for index, item in split_items(slot):
                ...
                if any(
                    f"注{note}" in text or f"注{note}" in quote for note in notes
                ):
                    anchored.append(
                        _suggestion(
                            slot,
    ...
    for slot in pool:
        for index, item in split_items(slot):
            ...
            if shared >= _MIN_SHARED_UNITS:
                ranked.append(
                    (shared, score, ratio, KIND_PRIORITY.get(str(item.get("kind") or ""), 0), slot, index)
                )
...
def map_question(
    ...
    for segment_index, segment in enumerate(segments, start=1):
        ...
        if not strong or has_value_marker:
            ...
            if len(source_ids) > 1 and any(
                marker in segment for marker in SOURCE_REF_MARKERS
            ):
    ...
    for facet in value_facets:
        ...
        for slot in pool:
            for index, item in split_items(slot):
                ...
                if not forms:
                    continue
                shared, score, ratio = overlap(
                    str(clause_text[facet["clause_id"]]), item, weights
                )
                candidates.append(
                    {
    ...
    for facet in facets:
        ...
        tail = (
            ...
            "须改选覆盖更全的 item、补标注，或显式 residual 说明"
            if union_missing
            else ""
        )
        extra = {
    ...
    if question.get("satisfy_rule") == "all" and len(source_ids) > 1:
        ...
        for source in sorted(source_ids - covered):
            ...
            _enqueue(
                ...
                "请从候选锚点中指定承载 item（或说明该来源不提供必需证据并给出理由）",
    ...
    if missing or len(required_keys) > _BROAD_LIMIT:
        ...
        result["machine_status_reason"] = (
            ...
            + (f" 且必需目标超护栏 {len(required_keys)}" if len(required_keys) > _BROAD_LIMIT else "")
...
def _decision_hint(item: Mapping[str, object]) -> str:
    ...
    if kind == "source_coverage" and item.get("facet_id") is None:
        return (
            ...
            "若要裁定该来源无需必需证据，须写明理由：____"
        )
    return (
        "`批准 / 改选 / 补标注 / 驳回`；chosen（可多选，按联合覆盖）：____；"
        "residual_accepted 与理由：____"
    )


def _render_suggestion(lines: list[str], suggestion: Mapping[str, object], index: int) -> None:
    role = "首选" if suggestion.get("role") == "primary" else "备选（仅供检索线索，不进可批准集合）"
    adequacy = "完整覆盖" if suggestion.get("adequacy") == "full" else "**部分覆盖**"
    lines.append(
...
def render_markdown(payload: Mapping[str, object]) -> str:
    ...
    for question in payload["questions"]:
        ...
        if question.get("unmatched_tokens"):
            lines.append(f"- **无承载 item 的数值（不得猜）**：{'、'.join(question['unmatched_tokens'])}")
        if question.get("weak_tokens"):
            lines.append(f"- 弱 token（年份/单字符，不参与匹配）：{'、'.join(question['weak_tokens'])}")
        if question.get("period_tokens"):
            lines.append(f"- 期间 token（已掩码，另作期间要件）：{'、'.join(question['period_tokens'])}")
...
def render_adjudication(payload: Mapping[str, object]) -> str:
    ...
    lines = [
        ...
        "- 机器候选规则 `evidence-mapping-5`：`role=required` 必需 ／ `role=supplementary` "
        ...
        "补人工标注，或显式声明 `residual_accepted=true` 并写明理由。`search_hint` 备选仅供检索线索。",
    ...
    for conflict in conflicts:
        ...
        lines.append(f"    - reviewer={conflict['reviewer']}；reviewed_at={conflict['reviewed_at']}")
    ...
    template = {
        ...
        "question_reviews": [
            {"query_id": "<题号>", "decision": "批准|需补要件",
             "reviewed_against_requirement": True, "reason": "..."}
        ],
        "facet_decisions": [
            {"item_id": "I32-<题号>-01", "facet_id": "<facet>", "query_id": "<题号>",
             "decision": "批准|改选|补标注|驳回",
             "chosen": [{"slot": "...", "item_index": 0}],
             "residual_accepted": False, "residual_reason": "",
             "supplement": {"slot": "...", "item_index": 3, "source_gold_revision": "..."},
             "reviewed_against_clause": "<段原文>", "reason": "..."}
        ],
        "negative_reviews": [
            {"query_id": "<负例题号>", "decision": "批准|待补",
             "full_text_coverage_confirmed": True, "reason": "..."}
        ],
    ...
    payload = {
        ...
        "contract": {
            ...
            "evidence_target_roles": {
                ...
                "adequacy=partial 的锚点须改选/补标注或显式 residual",
            },
            "structural_identity": (
