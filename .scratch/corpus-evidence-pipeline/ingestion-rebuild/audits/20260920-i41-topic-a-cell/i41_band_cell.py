"""I3-3 议题 A 收紧版 + S2 坐标投影接线：带区间取回（方案 C）+ cell 网格 row:/col:。

在 i40（band 收紧 11/11、EvidencePass 17/24、负例 OR 误报 6）基础上按用户裁决接线 S2：
1. **band 收紧**（与 i40 完全相同）：跨度上限方案 C（带内池块数上限 POOL_CAP=24），
   超限簇按池块锚拆子带；可证带宽上界 W_max = (POOL_CAP-1)*(G+1)+1+2*K = 49（G=1/K=1）。
2. **单变量重跑**（与 i40 完全相同）：负例走 OR 词元检索路径（不混入 S4 窄检索）。
3. **S2 坐标投影**（本迭代新增）：对已选带内的 table_row 单元建网格，派生 row:/col: 标签并
   发射 cell 级证据（FetchedEvidence locator 加入 page:/row:/col: token）。投影**只发生在
   已选中带内的块上**，不改变块选择——遵守议题 A 硬约束（带由 OR 词元命中池形成；不按
   金标词/页/行列补取；不做整篇文档取回）。

预期（与 i38 S2 层一致）：13 条 row:/col: 目标中 11 条转绿，2 条金标合成列标签
（industry-002 e2 col:2026E产能（配额）/ industry-003 e2 col:2026E产能）诚实失败。

诊断性质（非冻结回归）：
- 冻结资产逐字节核验；scorer 用工作树空白规约版（同 i38/i39/i40），血缘在内存补丁。
...
I40 = BASE / "audits/20260920-i40-topic-a-tighten"
...
BASE_PER_DOC = 8  # base 层：每文档前 8 块（与 i37/i40 对齐）
MAX_CHUNKS_PER_SOURCE = 300  # 每来源池块上限；超过即拒绝（cap saturation 纪律）

# 议题 A 主口径参数（与 i40 完全一致）
BAND_GAP = 1      # 相邻池块间允许的最大非池块数（空隙容忍）
BAND_EXPAND = 1   # 带两端各扩展块数
BAND_CAP = 8      # 每文档选带数上限（cap=8 的带语义）
# 跨度上限声明：方案 C（池块数上限）。带内池块 > POOL_CAP 时按池块锚拆分为子带。
POOL_CAP = 24
SPAN_MODE = "pool"
# 可证带宽上界（G=1/K=1）：W_max = (POOL_CAP-1)*(G+1)+1+2*K = 49
PROVABLE_WIDTH_BOUND = (POOL_CAP - 1) * (BAND_GAP + 1) + 1 + 2 * BAND_EXPAND
...
def cap_split(bands: list[dict], score_by_pos: dict[int, float], n: int,
              pool_cap: int, expand: int) -> list[dict]:
    """方案 C：带内池块数上限。簇内池块 > pool_cap 时按池块锚顺序拆为子带。

    每子带 = 连续 ≤pool_cap 个池块 ± expand 扩展；子带分 = 子带内最大 ts_rank。
    可证带宽上界：子带区间宽 ≤ (pool_cap-1)*(gap+1)+1+2*expand（gap 为聚簇空隙容忍）。
    """
    out = []
    for band in bands:
        bp = band["pool"]
        ...
        for i in range(0, len(bp), pool_cap):
            ...
            out.append({"start": max(0, group[0] - expand),
...
def build_observation_band(query_id, doc_hits: dict, bands_by_source: dict,
                           fetch_cid, aliases, with_s2: bool = False):
    """按已选带装配观测：每带拼接为单一 FetchedEvidence + 可选 S2 cell 证据。

    ``bands_by_source``: {source_id: [band, ...]}（已按带分降序取前 BAND_CAP 个；
    每带含 ``chunk_ids`` = 原文序带内全部块 id，含扩展的非池块）。

    ``with_s2=True`` 时对带内 chunk 的 table_row 单元派生 row:/col: 标签并发射 cell 证据
    （仅页级证据之外的增量；不改变块选择，仅增加坐标粒度）。
    """
    ...
    for source_id, bands in bands_by_source.items():
        ...
        for band in bands:
            ...
            if with_s2:
                evidences.extend(emit_cell_evidence(band_chunks))
...
def main() -> int:
    # ── 1. 冻结资产核验（除工作树 scorer 外与 r39 一致，同 i37/i38/i40 口径）──
    ...
    write_once("binding.json", {
        ...
        "band_semantics": {
            ...
            "span_cap_mode": SPAN_MODE, "pool_cap": POOL_CAP,
            "provable_width_bound": PROVABLE_WIDTH_BOUND,
            "cap_is_band_count": "max_chunks_per_top_document(8) 语义改为带数；不调 cap 值",
            "selection": "带由 OR 词元命中池形成；不按金标词/页/行列补取",
            "single_variable": "负例走 OR 词元检索路径（不混入 S4 窄检索）",
            "s2": "对已选带内 chunk 的 table_row 单元派生 row:/col: 并发射 cell 证据（不改变选择）",
        },
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        # kept 单元按 build → page 缓存（与 i35/i37/i40 同口径：仅 KEPT）
        ...
        obs_band_s2: dict[str, object] = {}
        band_trace: dict[str, object] = {}
        # 体积统计（有答案题带选择；S2 不改变选择，与 i40 应字节一致）
        volume: list[dict] = []
        ...
        for question in questions:
            ...
            if question.answer_existence is AnswerExistence.NO_ANSWER:
                # 负例（单变量）：与有答案题同一 OR 检索；FP 判定只取决于是否有文档被检索。
                grouped = calibrate.group_hits(hits, policy.top_k, BASE_PER_DOC)
                obs = build_observation_base(question.query_id, grouped, fetch, aliases)
                obs_base[question.query_id] = obs
                obs_band[question.query_id] = obs
                obs_band_s2[question.query_id] = obs
            ...
            for source_id in grouped_base:
                ...
                # 跨度上限（方案 C）：按池块数拆分超限簇
                bands = cap_split(bands, score_by_pos, len(ordered), POOL_CAP, BAND_EXPAND)
                ...
                covered = set()
                for band in selected:
                    covered.update(range(band["start"], band["end"] + 1))
                widths = [band["end"] - band["start"] + 1 for band in selected]
                volume.append({"query_id": question.query_id, "source": source_id,
                               "n": len(ordered), "selected_bands": len(selected),
                               "sum_widths": sum(widths), "union": len(covered),
                               "ratio": round(len(covered) / len(ordered), 4)})
            ...
            obs_band[question.query_id] = build_observation_band(
                ...
                with_s2=False)
            obs_band_s2[question.query_id] = build_observation_band(
                question.query_id, doc_hits, bands_by_source, fetch_cid, aliases,
                with_s2=True)
            traces.append({"query_id": question.query_id,
                           "answer_existence": question.answer_existence.value,
        ...
        for q in questions:
            ...
            for target in q.evidence_targets:
                ...
                if layer is not None:
                    ...
                    continue
                row["matched"] = False
                sel_evs = [ev for doc in (obs_q.documents if obs_q else ())
                           for ev in doc.evidence]
                selected_text = "\n".join(ev.text for ev in sel_evs)
                in_selected = norm(target.quote) in norm(selected_text)
                in_single = any(norm(target.quote) in norm(ev.text) for ev in sel_evs)
        ...
        # ── 4. 议题 A：11 条带覆盖可行性（含敏感性扫描；选择不变，与 i40 应一致）──
        ...
        for qid, tid in TOPIC_A_TARGETS:
            ...
            for gap, expand in SENSITIVITY:
                ...
                # 跨度上限（方案 C）：按池块数拆分超限簇（与主循环同一规则）
                bands = cap_split(bands, score_by_pos, len(ordered), POOL_CAP, expand)
                ranking = rank_bands(bands)
                ...
                width_bound = (POOL_CAP - 1) * (gap + 1) + 1 + 2 * expand
                sensitivity.append({"gap": gap, "expand": expand,
                                    "coverable": True, "rank": rank,
                                    "band_span": [bands[cover_band]["start"],
                                                  bands[cover_band]["end"]],
                                    "band_width": bands[cover_band]["end"]
                                    - bands[cover_band]["start"] + 1,
                                    "band_pool": len(bands[cover_band]["pool"]),
                                    "provable_width_bound": width_bound,
                                    "selected_in_band_cap": sel,
                                    "splice_contains_quote": splice_ok})
            entry["sensitivity"] = sensitivity
            feasibility.append(entry)

        # ── 5. 负例（no-answer）结果：单变量（OR 路径，与有答案题同一管线）──
        neg = []
        for q in questions:
            o = obs_band_s2[q.query_id]
        ...
        write_once("i41-trace.json", traces)
        write_once("i41-band-trace.json", band_trace)
        write_once("i41-observations.json", [asdict(o) for o in obs_band_s2.values()])
        write_once("i41-feasibility.json", feasibility)
        write_once("i41-score.json", asdict(report_band_s2))
        (HERE / "i41-score.md").write_text(format_report(report_band_s2) + "\n")
        write_once("fetch-receipts.json", receipts)
        write_once("i41-diagnostics.json", diagnostics)
        write_once("negative-cases.json", {
            "note": "no-answer questions under OR token retrieval (single-variable; "
                    "no S4 narrow retrieval in this run)",
            "cases": neg})
        ...
        # 体积上界统计（与 i40 同口径）
        vol_sum = {"max_band_width_measured": max_band_width_measured,
                   "provable_width_bound": PROVABLE_WIDTH_BOUND,
                   "width_bound_ok": max_band_width_measured <= PROVABLE_WIDTH_BOUND,
                   "per_doc": volume}
        if volume:
            vol_sum["max_union"] = max(v["union"] for v in volume)
            worst = max(volume, key=lambda v: v["ratio"])
            vol_sum["worst_ratio"] = {"query_id": worst["query_id"], "source": worst["source"],
                                      "n": worst["n"], "union": worst["union"],
                                      "ratio": worst["ratio"]}
            flagged = [v for v in volume if v["query_id"] in
                       ("company-004", "company-005", "company-008", "industry-001")]
            vol_sum["flagged_queries"] = flagged

        # S2 row:/col: 目标分类
        rowcol_targets = [d for d in diagnostics if any(
            t.startswith("row:") or t.startswith("col:") for t in d["locator"])]
        rowcol_still_failed = [
            {"query_id": d["query_id"], "target_id": d["target_id"], "locator": d["locator"],
             "in_single_evidence": d.get("in_single_evidence")}
            for d in rowcol_targets if not d["matched"]]

        summary = {
            "artifact": "i4-1-topic-a-cell",
            ...
            "band_params": {"gap": BAND_GAP, "expand": BAND_EXPAND, "band_cap": BAND_CAP,
                            "span_cap_mode": SPAN_MODE, "pool_cap": POOL_CAP,
                            "provable_width_bound": PROVABLE_WIDTH_BOUND},
            "single_variable": "negative questions use OR token retrieval (no S4)",
            "s2": "cell grid row:/col: projection on selected band chunks (no selection change)",
            "layers": {
                "base": layer_summary(report_base),
                "band": layer_summary(report_band),
                "band_s2": layer_summary(report_band_s2),
            },
            "negative": {"or_false_positives": report_band_s2.false_positives},
            "volume": vol_sum,
            "target_buckets_final": dict(buckets),
            "row_col": {
                "total": len(rowcol_targets),
                "matched": sum(1 for d in rowcol_targets if d["matched"]),
                "still_failed": rowcol_still_failed,
            },
            "topic_a": {
        ...
        write_once("i41-summary.json", summary)
        ...
        lines = [
            ...
            f"- **跨度上限声明：方案 C（带内池块数上限 POOL_CAP={POOL_CAP}）**；超限簇按池块锚拆子带",
            f"- 可证带宽上界：W_max = (POOL_CAP-1)*(G+1)+1+2*K = **{PROVABLE_WIDTH_BOUND} 块**（G=1/K=1）",
            f"- 体积实测：最大带宽 {max_band_width_measured} ≤ {PROVABLE_WIDTH_BOUND}（{'满足' if max_band_width_measured <= PROVABLE_WIDTH_BOUND else '违反'}）",
        ...
        for label, rep in (("base（前8块·页级）", report_base),
                           ("band（带区间取回·带数8·POOL_CAP=24）", report_band),
                           ("band+S2（+cell 网格 row:/col:）", report_band_s2)):
        ...
        lines += [
            ...
            "## S2 坐标投影（row:/col: 目标）",
            "",
            f"- 总数：{summary['row_col']['total']}；转绿：{summary['row_col']['matched']}；仍失败：{len(summary['row_col']['still_failed'])}",
            "",
        ]
        for d in rowcol_targets:
            status = "matched" if d["matched"] else "still_fail"
            lines.append(f"- {d['query_id']} {d['target_id']} {d['locator']} → {status}"
                         f"{'（layer=' + d['layer'] + '）' if d['matched'] else ''}")
        if summary["row_col"]["still_failed"]:
            lines += [
                "",
                "仍失败的 row:/col: 目标（金标合成列标签，cell 网格无法派生等义 token）：",
                "",
            ]
            for item in summary["row_col"]["still_failed"]:
        ...
        for f in feasibility:
            ...
            lines.append(
                ...
                f"| {sens['rank']} | {sens.get('band_width', '-')} "
                f"| {sens['selected_in_band_cap']} "
        ...
        for gap, expand in SENSITIVITY:
            ...
            bound = (POOL_CAP - 1) * (gap + 1) + 1 + 2 * expand
            lines.append(f"- (G={gap}, K={expand})：{cnt}/11 进带数 8；可证带宽上界 {bound}")
        lines += [
            "",
            "## 体积上界（方案 C，POOL_CAP=24，G=1/K=1）",
            "",
            f"- 可证带宽上界：**{PROVABLE_WIDTH_BOUND} 块/带**（推导：每带 ≤24 池块，相邻池块间隔 ≤G+1=2，"
            "加两端扩展 K=1）",
            f"- 实测最大带宽：**{max_band_width_measured} ≤ {PROVABLE_WIDTH_BOUND}**",
            f"- 聚合上界：每文档选中块数 ≤ min(n, {BAND_CAP}×{PROVABLE_WIDTH_BOUND}={BAND_CAP * PROVABLE_WIDTH_BOUND})",
        ]
        if volume:
            worst = vol_sum["worst_ratio"]
            lines.append(f"- 实测最差聚合（占比）：{worst['query_id']} → 文档 {worst['source'][:12]}… "
                         f"n={worst['n']} 选中 {worst['union']} 块（{worst['ratio']*100:.1f}%）")
            maxu = max(volume, key=lambda v: v["union"])
            lines.append(f"- 实测最大聚合（绝对块数）：{maxu['query_id']} → 文档 {maxu['source'][:12]}… "
                         f"n={maxu['n']} 选中 {maxu['union']} 块（{maxu['ratio']*100:.1f}%）")
        lines += [
            "",
        ...
        for q in ("company-004", "industry-001"):
            es = [v for v in volume if v["query_id"] == q]
            if not es:
                continue
            best = max(es, key=lambda v: v["union"])
            pct = {"company-004": "52%", "industry-001": "70%"}[q]
            lines.append(f"| {q} | {best['n']} | {pct}（i39 未收紧） | "
                         f"{best['union']} | {best['ratio']*100:.1f}% |")
        lines += [
            "",
            "## 负例（单变量：OR 检索路径）",
            "",
        ...
        for case in neg:
            lines.append(f"- {case['query_id']}：OR 检索命中文档 {case['retrieved_documents']}（期望 0）"
                         + (" —— 误报" if case["retrieved_documents"] else ""))
        (HERE / "i41-report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    return 0
