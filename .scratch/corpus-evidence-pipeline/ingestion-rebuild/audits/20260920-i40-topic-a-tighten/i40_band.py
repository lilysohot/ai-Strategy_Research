"""I3-3 议题 A 收紧版实测：带跨度上限（方案 C=池块数上限）+ 单变量重跑。

在 i39（band 可行性 11/11、EvidencePass 17/24）基础上按用户裁决收紧：
1. **band 收紧**：声明跨度上限为**方案 C（带内池块数上限 POOL_CAP=24）**。
   超出则按池块锚拆分（cap_split），带排名仍取簇内最大 ts_rank。可证带宽上界：
   W_max = (POOL_CAP-1)*(G+1)+1+2*K（G=1/K=1 时 = 49），实测最大带宽须 ≤ W_max。
2. **单变量重跑**：负例（no-answer）改回 **OR 词元检索路径**（与有答案题同一检索
   管线，不混入 S4 窄检索），如实报告 band 层误报数与 EvidencePass。
3. **不做整篇文档取回**：只取回带区间内块；聚合选中量如实报告（含最差文档占比）。

诊断性质（非冻结回归）：
- 冻结资产逐字节核验；scorer 用工作树空白规约版（同 i38/i39），血缘在内存补丁。
- 只读 PG + 0 model_calls；产物 write-once 写入本目录。

议题 A 语义（topic-a-chunk-evidence-granularity.md §4）保持不变：
- 不调 cap 值（8=带数）；带由 OR 词元命中池形成；不按金标词/页/行列补取。
"""
...
def main() -> int:
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        write_once("i40-trace.json", traces)
        write_once("i40-band-trace.json", band_trace)
        write_once("i40-observations.json", [asdict(o) for o in obs_band.values()])
        write_once("i40-feasibility.json", feasibility)
        write_once("i40-score.json", asdict(report_band))
        (HERE / "i40-score.md").write_text(format_report(report_band) + "\n")
        write_once("fetch-receipts.json", receipts)
        write_once("i40-diagnostics.json", diagnostics)
        ...
        summary = {
            ...
            "negative": {"or_false_positives": report_band.false_positives},
            "volume": vol_sum,
        ...
        write_once("i40-summary.json", summary)
