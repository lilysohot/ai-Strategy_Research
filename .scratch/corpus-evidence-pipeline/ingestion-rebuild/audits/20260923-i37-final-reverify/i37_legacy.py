"""I3-7 最终重验：旧检索/财务非回归（旧检索 golden / 财务 / 公式 / 客户表 / 正文 / 宏观 / 负控 / 审批契约门）。

母本：audits/20260923-i35-legacy-nonregress/i35_replay.py（I3-5）；本脚本按 I3-6 冻结
manifest 的 m4_m5_gates_to_reverify 范围原样复跑，仅改产物目录/文件名与口径注记，
判定逻辑与 I3-5 完全一致（I3-7 适配器逐字节语义等价）。
...
用法：uv run python i37_legacy.py（仓库根目录执行）
"""
...
AUDIT = ING / "audits/20260923-i37-final-reverify"
...
def main() -> int:
    ...
    results["overall"] = {
        ...
        "note": "golden 逐题结果单列；I3-7 重验不构成 M6 最终放行（须另走独立复核 + U 签认）",
    }

    out = AUDIT / "i37-legacy-results.json"
    if out.exists():
