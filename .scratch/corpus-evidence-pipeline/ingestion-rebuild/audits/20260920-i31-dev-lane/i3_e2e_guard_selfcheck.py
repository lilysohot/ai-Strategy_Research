...
2026-09-20 说明：本文件是 r32 版生成器（``audits/20260919-i3-1-e2e/i3_e2e_guard_selfcheck.py``）
的**同一生成逻辑副本**，仅为「允许路径 6→8（dev lane）」的守卫变更在本轮重新出报告；
写入目标随目录改为本目录（旧报告 write-once 保留，不覆盖）。

用法::

    env -u PYTHONPATH uv run python \
        .scratch/corpus-evidence-pipeline/ingestion-rebuild/audits/20260920-i31-dev-lane/i3_e2e_guard_selfcheck.py
"""
