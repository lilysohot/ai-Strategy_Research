...
def region_review(target, path, **overrides):
    from plugins.corpus.preparation.gap_review import REGION_SCHEMA_REV

    data = signed_record(target).as_payload()
    data.update(
        schema_rev=REGION_SCHEMA_REV,
        required_locators=["page:1", "page:2"],
        region_source_path=str(path),
        region_targets={"page:2": ["Supplemental illustration, unrelated to page one evidence."]},
    )
    data.update(overrides)
    return GapReview.from_json(json.dumps(data))


def test_same_page_region_review_publish_and_recheck_source(built_pdf, tmp_path):
    from plugins.corpus.preparation.engine import (
        EngineError,
        check_build_publishable,
        publish_build,
    ...
    store, target = built_pdf
    path = tmp_path / "synthetic-review.pdf"
    before = target.quality_report
    review = region_review(target, path)
    store.put_gap_review(review)
    publication = publish_build(store, target.build_id, activated_at=NOW)
    assert publication.active_build_id == target.build_id
    assert store.get_build(target.build_id).quality_report == before
    assert store.get_gap_review(target.build_id).review_id == review.review_id
    path.write_bytes(b"changed source")
    with pytest.raises(EngineError, match="source bytes"):
        check_build_publishable(store, target.build_id)
...
def test_region_proof_fails_closed(built_pdf, tmp_path, fault):
    store, target = built_pdf
    path = tmp_path / "synthetic-review.pdf"
    kept = store.get_units(target.build_id)
    record = region_review(target, path)
    if fault in {"missing_bbox", "nonfinite", "forged_bbox"}:
        box = {
            "missing_bbox": None,
            "nonfinite": (0, 0, float("nan"), 10),
            "forged_bbox": (0, 0, 10, 10),
        }[fault]
        kept = tuple(
            replace(u, location=replace(u.location, bbox=box)) if u.location.page == 2 else u
            for u in kept
        )
    elif fault == "missing_quote":
        record = region_review(target, path, region_targets={"page:2": ["Missing evidence"]})
    elif fault == "wrong_source":
        path.write_bytes(b"different bytes")
    elif fault == "missing_source":
        path.unlink()
    elif fault == "other_gap_code":
        other = "issue:table_lines_without_extraction:page:2"
        target = replace(
            target, quality_report=json.dumps({"gap_regions": [other], "oversized_chunks": []})
        )
        record = region_review(target, path, gaps={other: "Synthetic signed explanation"})
    from plugins.corpus.preparation.engine import gap_records_of

    with pytest.raises(GapReviewError):
        apply_gap_review(record, target, kept, gap_records_of(target))


@pytest.mark.parametrize("rectangle", [(30, 60, 560, 700), (30, 200, 560, 700)])
def test_all_image_placements_checked_without_selecting_convenient_one(tmp_path, rectangle):
    import pymupdf

    # First placement is harmless. The second overlaps required text in one case.
    path = tmp_path / "regions.pdf"
    with pymupdf.open() as pdf:
        page = pdf.new_page()
        page.insert_text((60, 80), "Required evidence")
        page.insert_image(pymupdf.Rect(30, 300, 560, 700), stream=b"P6\n1 1\n255\n\xff\x00\x00")
        page.insert_image(pymupdf.Rect(rectangle), stream=b"P6\n1 1\n255\n\x00\xff\x00")
        box = tuple(page.get_text("blocks")[0][:4])
        pdf.save(path)
    key = "issue:image_region_unreadable:page:1"
    target = replace(
        build(),
        source_id=sha256_of_bytes(path.read_bytes()),
        quality_report=json.dumps({"gap_regions": [key], "oversized_chunks": []}),
    )
    unit = Unit(
        "u",
        target.build_id,
        "paragraph",
        "Required evidence",
        sha256_of_bytes(b"Required evidence"),
        location=UnitLocation(page=1, bbox=box),
    )
    record = region_review(
        target,
        path,
        gaps={key: "Human checked image content"},
        required_locators=["page:1"],
        region_targets={"page:1": ["Required evidence"]},
    )
    if rectangle[1] == 60:
        with pytest.raises(GapReviewError, match="intersect"):
            apply_gap_review(record, target, [unit], gap_records([key]))
    else:
        assert (
            apply_gap_review(record, target, [unit], gap_records([key]))[0].lifecycle
            is GapLifecycle.ACKNOWLEDGED
        )


def test_legacy_credentials_remain_byte_stable_and_page_only():
    record = signed_record(policy_rev="gap-policy-2")
    payload = record.as_payload()
    assert "region_targets" not in payload and "region_source_path" not in payload
    assert GapReview.from_json(record.to_json()).review_id == record.review_id
    with pytest.raises(GapReviewError):
        signed_record(policy_rev="gap-policy-2", required_locators=["page:2"])
        apply_gap_review(
            signed_record(policy_rev="gap-policy-2", required_locators=["page:2"]),
            build(),
            units(),
