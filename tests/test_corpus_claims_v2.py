"""D2 Claims v2 验收：影子表、证据、规范化、lint 和兼容 Adapter。"""
...
def v2_svc():
    ...
    try:
        ...
        pytest.skip(f"PG 不可用，跳过 D2 v2 测试：{exc}")

    schema = f"corpus_d2v2_{uuid.uuid4().hex[:8]}"
            ...
            with conn.cursor() as cur:
                ...
                cur.executemany(
                    ...
                    [
                        ...
                        ("d2v2_doc", 2, "p2", "8月 销量 10 万台，环比上升。"),
                        ("d2v2_doc", 3, "p3", "目标价 1888 元。"),
                    ],
                )
            conn.commit()
        yield svc
...
def test_extract_claims_v2_persists_shadow_rows_and_default_projection(
    v2_svc: CorpusService,
) -> None:
    responses = [
        {
            "claim_text": "营业收入 1741 亿元",
        ...
        {
            "claim_text": "8月 销量 10 万台",
            "evidence_quote": "8月 销量 10 万台",
            "scope": "company",
            "subject": "600519.SH",
            "metric": "销量",
            "kind": "fact",
    ...
    stats = v2_svc.extract_claims_v2(llm=llm, retry_attempts=1, sleep_between=0)
    assert stats.failed == 0, stats.as_dict()
    assert (stats.claims, stats.review, stats.rejected) == (1, 1, 1)

    ok_rows = v2_svc.claims_v2_of(limit=10)
    assert len(ok_rows) == 1
    assert ok_rows[0]["quality_status"] == "ok"
    assert ok_rows[0]["evidence_quote"] == "营业收入 1741 亿元"

    all_rows = v2_svc.claims_v2_of(quality_status=None, limit=10)
    assert {row["quality_status"] for row in all_rows} == {"ok", "review", "rejected"}
    ...
    legacy = v2_svc.legacy_claims_from_v2(limit=10)
    assert len(legacy) == 1
    assert legacy[0].tickers == ("600519.SH",)
    assert legacy[0].metric == "营业收入"


def test_v2_backup_columns_are_registered() -> None:
    assert "claims_v2" in CorpusService._BACKUP_COLUMNS
    assert "claim_block_runs_v2" in CorpusService._BACKUP_COLUMNS
    assert "evidence_quote" in CorpusService._BACKUP_COLUMNS["claims_v2"]
    assert "diagnostics" in CorpusService._BACKUP_COLUMNS["claim_block_runs_v2"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (CLAIMS_V2_SQL, CLAIMS_V2_COMMENTS, "claims_v2"),
        (CLAIMS_V2_SQL, CLAIMS_V2_COMMENTS, "claim_block_runs_v2"),
    ],
)
