...
状态：本 v1 保留为可选的严格事件研究契约，不再作为研报宏观数据的默认准入条件。
[M1 输入模型](corpus-macro-input-models.md) 已完成；M2 实现保留，官方网页真实验收按用户决定暂跳过；M3—M6 未执行。

## 当前适用范围调整（2026-09-13）

用户决定：研报宏观数据不默认逐条回查官网；政府统计与行情由用户提供/确认的同花顺接口承担。
材料层保留观点、论据和视角，数据推导由数据层承担；材料数字不自动晋升为数据层输入。
已有财务来源内部复算可保留，但不改变其来源归属，不代表独立宏观数据计算能力。
不宣称研报事实已经外部证实，不补造未知版本、年份或时间；引用和不依赖精确时点的推导不强制要求官方归档。

本文件以下第 1—7 节继续描述“官方首发实际值与事前预期”的严格用途。
其中官方来源核验可以由后续选定、满足版本/时间证据要求的第三方数据接口提供，不要求一定爬 BLS 网页。
材料内部复算不冒用 `nfp_surprise_delta_v1` 的严格事件声明；严格计算仅在数据契约与所需快照具备后按需实施。
本次只调整文档和优先级，未放宽现有运行时门禁或注册公式。当前执行顺序以
[统一总计划](plan/claims-market-closed-loop-plan.md) 的最新决策为准；[宏观专项](plan/corpus-macro-contract-implementation.md) 保留为历史实现记录。
...
2026-09-13：实施依赖、交付、验收、数据选择与回滚已拆分为
[M1—M6 实施清单](plan/corpus-macro-contract-implementation.md)。现有局部门禁修复不代表 M1—M6 已完成。
