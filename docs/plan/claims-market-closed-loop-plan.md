...
| I0—I5 | [语料基础链路重构](corpus-ingestion-rebuild-architecture.md)                                                                                                                                                                              | PR-DATA-01/02/09/12、PR-OUT-04/06/07 | 三类范围已明确；候选设计待 I0 证据复核        | **I0-A 交付齐（M1 达成）、I0-B 完成（M2 证据齐备）、I0-C 完成（M3 达成）、I1/M4 放行，I2-1/I2-2/I2-3 完成且 I2-2 独立复核整改已回填；M5—M8 未放行**；强制零模型，生产库未写入/未清库，不依赖 R2 语义通过 |
...
| I3 | 三类各至少两份开发材料的 CLI/工具闭环、校准与 I3-7 最终重验                           | I2 + 先冻结来源/查询/证据 gold 与评分器     | 待执行；最终版本逐类指标、关键题、旧检索/财务及负例全满足，不继承校准前结果，不等待 R2                                                                                                                                                                                                                                                                                                                                                  |
