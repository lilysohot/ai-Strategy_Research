...
> **已实施记录 / 2026-09-13 范围澄清**：本文保留既有行情、历史、财务工具的技术设计和当时验证。
> 后续执行顺序以 [统一总计划](claims-market-closed-loop-plan.md) D1/D2/A1/V1 为准；下文旧 M 编号不是新的总清单。
> 用户确认政府统计和股票行情由其同花顺接口承担，但不能据此认定历史记录中的供应商已覆盖所有新端点。
> D1 需核对实际接口、权限、实时性与版本覆盖；不重建已有市场工具、不把研报数字写入数据层。
> 本次仅改文档，未重新确认凭据、在线能力或下载第三方文档；下文供应商能力描述属于原实施时期记录。
