...
本文大量复用了既有设计稿 `docs/tittel/fin-research-three-layer-architecture.md` 的 L1/L2/L3
分层与精度红线，并针对「多源 + 高时效 + 交易挖掘」三个新特性做了扩展。
...
原方案 `docs/tittel/fin-research-three-layer-architecture.md:247` 的红线假设「检索能给我正确候选，
只是不能引用 snippet」。但真实情况**检索层本身就把数字错误的候选排到了前面**——红线挡不住
「取证了一个语义正确、数字过期的原文」。
