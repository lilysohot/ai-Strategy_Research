...
REGION_SCHEMA_REV = "human-gap-review-2"
...
class GapReview:
    ...
    region_source_path: str = ""
    region_targets: tuple[tuple[str, tuple[str, ...]], ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        supported = {
            (SCHEMA_REV, "gap-policy-2"),
            (SCHEMA_REV, GAP_POLICY_REV),
            (REGION_SCHEMA_REV, GAP_POLICY_REV),
        }
        if (self.schema_rev, self.policy_rev) not in supported:
            raise GapReviewError("unknown or stale gap review schema/policy_rev")
        if self.schema_rev == SCHEMA_REV:
            if self.region_targets or self.region_source_path:
                raise GapReviewError("page-only schema cannot carry region refinements")
        else:
            _text(self.region_source_path, "region_source_path")
            if not isinstance(self.region_targets, tuple) or not self.region_targets:
                raise GapReviewError("region schema requires complete page evidence quotes")
            for locator, quotes in self.region_targets:
                if _coordinate(locator).kind is not GapCoordinateKind.PAGE:
                    raise GapReviewError("region refinement requires a page locator")
                if locator not in self.required_locators:
                    raise GapReviewError("region page is outside signed required locators")
                if not isinstance(quotes, tuple) or not quotes or len(set(quotes)) != len(quotes):
                    raise GapReviewError("region quotes must be nonempty and unique")
                for quote in quotes:
                    _text(quote, "region quote")
            if len(dict(self.region_targets)) != len(self.region_targets):
                raise GapReviewError("duplicate region page")
    ...
    def from_json(cls, payload: str) -> GapReview:
        ...
        try:
            ...
            if data.get("schema_rev") == SCHEMA_REV:
                fields -= {"region_source_path", "region_targets"}
            if set(data) != fields:
                raise GapReviewError("gap review fields do not match schema")
            ...
            if "region_targets" in data:
                targets = data["region_targets"]
                if not isinstance(targets, dict) or not all(
                    isinstance(q, list) for q in targets.values()
                ):
                    raise GapReviewError("invalid region targets shape")
                data["region_targets"] = tuple(sorted((p, tuple(q)) for p, q in targets.items()))
            return cls(**data)
    ...
    def as_payload(self) -> dict[str, object]:
        ...
        if self.schema_rev == SCHEMA_REV:
            del payload["region_source_path"]
            del payload["region_targets"]
        else:
            payload["region_targets"] = {p: list(q) for p, q in self.region_targets}
        return payload
...
def apply_gap_review(
    ...
    refined_pages: set[int] = set()
    if review.region_targets:
        from plugins.corpus.preparation.pdf_gap_regions import verify_region_targets

        try:
            refined_pages = verify_region_targets(review, kept)
        except (OSError, ValueError, RuntimeError, ImportError) as exc:
            raise GapReviewError(f"PDF region proof rejected: {exc}") from exc
    for record in blocking:
        ...
        if not all(
            _disjoint(coordinate, target)
            or (
                record.key == f"issue:image_region_unreadable:page:{coordinate.page}"
                and coordinate.kind is target.kind is GapCoordinateKind.PAGE
                and coordinate.page == target.page
                and coordinate.page in refined_pages
            )
