"""Claims 细节规则底座：确定性规范化、triage、lint 与 ClaimRecord（I2-7 拆分）。

原 claims_v2（影子抽取路线，D2 v2 两表零行、未启动）随消费者矩阵裁决退役：
影子表 DDL 不再由本模块携带；本模块只承载仍被证据链/审计/语义消费的共享规则
（ClaimRecord、records_from_payload、apply_lint、classify_doc_kind_detail、
triage_block_detail 等）。财务公式与用途验证继续经同一套来源证据消费这些规则。
"""
...
def _split_coordinate(
    ...
    if scope == "macro":
        ...
        subject = {"美国": "US", "US": "US", "USA": "US", "U.S.": "US", "United States": "US"}.get(
            subject or "", subject
        )
        metric = {
            "新增非农就业": "NFP",
